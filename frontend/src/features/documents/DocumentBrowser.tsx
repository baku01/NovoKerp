...
const FolderItem: React.FC<{ name: string, parentPath: string }> = ({ name, parentPath }) => {
    ...
    const handleFileClick = (fileName: string) => {
        ...
        const relativePath = `Docs/${fullPath}/${fileName}`;

        if (baseUrl) {
            window.open(baseUrl + relativePath, '_blank');
        } else {
            console.warn("Base URL not found for file download", relativePath);
            alert(`Não foi possível determinar a URL de download para: ${fileName}`);
        }
    };
