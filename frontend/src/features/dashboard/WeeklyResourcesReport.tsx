...
export const WeeklyResourcesReport: React.FC = () => {
    ...
    const tableRows = useMemo(() => {
        ...
        const activeTypes = resourceTypes.filter(t => 
             selectedTypeSglas.length === 0 || selectedTypeSglas.includes(t.fu_sgla)
        );
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 p-4 space-y-4 overflow-y-auto">
            ...
            <div className="bg-white p-4 rounded-lg shadow space-y-4">
                <h1 className="text-xl font-bold text-slate-800">Programação Semanal de Recursos</h1>
            ...
            <div className="bg-white p-4 rounded-lg shadow overflow-x-auto">
                 {isLoading ? (
                     ...
                     <div className="text-center py-8 text-slate-500">Nenhum dado encontrado para os filtros selecionados.</div>
                 ) : (
                            ...
                            <tr>
                                ...
                                {updateDates.map(d => (
                                    <th key={d.toISOString()} className="px-4 py-2 border text-center">
                                        ...
                                        <span className="text-[10px] text-slate-400">{format(d, 'EEE', { locale: undefined })}</span> 
                        ...
                        <tbody>
                            {tableRows.map(row => (
                                <tr key={row.sgla} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-4 py-2 border text-center bg-blue-50 font-semibold">{row.realQty}</td>
