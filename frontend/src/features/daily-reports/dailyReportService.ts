...
export async function fetchRdoOrdersByDate(
  ...
  return callProcedure<RdoOrder>('pesquisaOrdensServicoDataApontamentoRdo', params);
}
...
export async function updateRdoFinalization(
  empresa: string,
  worksiteId: number,
  date: Date,
  finalized: boolean,
  ...
  const params = [
    ...
    createParam('ldApData', 'SmallDatetime', format(date, 'yyyy-MM-dd')),
    createParam('lnIdOrds', 'Int', orderId || null),
    createParam('lnApFnlz', 'Int', finalized ? 1 : 0),
  ];
  return callProcedure('atualizaApontamentoFinalizado', params);
}
